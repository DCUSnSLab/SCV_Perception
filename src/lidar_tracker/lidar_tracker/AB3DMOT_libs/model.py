...
# model.py - AB3DMOT clean version

import numpy as np
from .box import Box3D
from .kalman_filter import KalmanBoxTracker
from .matching import hungarian_matching, greedy_matching
from .xinshuo_miscellaneous import print_log
...
class AB3DMOT(object):
    def __init__(self, cfg, cat):
        ...
        self.affinity_threshold = cfg.affinity_threshold
        self.trackers = []
        self.frame_count = 0
        self.reorder = [3, 4, 5, 6, 2, 1, 0]
        self.affi_process = cfg.affi_pro 
    ...
    def update(self, dets_all, info):
        self.frame_count += 1
        
        # 1. 입력 데이터 (Numpy Array)
        # [h,w,l,x,y,z,theta] 순서
        dets = dets_all

        # 2. 기존 트래커 상태 예측 (Prediction)
        to_del = []
        trks = []
        for t, trk in enumerate(self.trackers):
            pos = trk.predict() # Kalman Filter Predict 결과 (배열)
            
            # [수정된 부분] 
            # 에러 원인: trk.ry 같은 속성이 없음.
            # 해결: pos 배열(칼만필터 상태)을 그대로 사용.
            # pos 순서는 kalman_filter.py에 따라 [h, w, l, x, y, z, theta, ...] 임
            
            # 예측된 상태값 중 앞 7개(기하 정보)만 가져옴
            # [h, w, l, x, y, z, theta]
            trk_pos = pos[:7].reshape(-1) 
        ...
        # 3. 매칭 (Hungarian Algorithm)
        matched, unmatched_dets, unmatched_trks = hungarian_matching(
            dets, trks, self.cfg.affi_pro_type, self.affinity_threshold, self.ego_com
        )
        ...
        for trk in reversed(self.trackers):
            ...
            if (trk.time_since_update < 1) and (trk.hit_streak >= self.min_hits or self.frame_count <= self.min_hits):
                # 결과 포맷: [h, w, l, x, y, z, theta, id, score]
                # 여기서 score는 info 딕셔너리에 저장된 값을 가져옴
                score = trk.info.get('score', 0.0) if hasattr(trk, 'info') and trk.info else 0.0
                
                # 리턴값 생성
                res = np.concatenate((d, [trk.id + 1], [score])).reshape(1, -1)
                ret.append(res)
