...
class ObjectDepthTracker:
    def __init__(self):
        ...
        filter_name = rospy.get_param("~filter_type", "kalman_ca9d")
        ...
        self.depth_scale = rospy.get_param("~depth_scale", 1.9)
        self.clip_lower_pct = rospy.get_param("~clip_lower_pct", 5.0)   # 하위 %
        self.clip_upper_pct = rospy.get_param("~clip_upper_pct", 95.0)  # 상위 %
        rospy.loginfo("Depth scale = %.4f", self.depth_scale)
    ...
    def sync_cb(self, yolo_msg, depth_msg, caminfo_msg):
        ...
        for det_id, det in enumerate(yolo_msg.detections.detections):
            ...
            # ★ (A) 5–95 % 클리핑으로 경계·노이즈 제거
            valid_z = z_vals[good]
            if valid_z.size < 30:
                continue
            low, high = np.percentile(     # 비율은 파라미터화
                valid_z, 
                [self.clip_lower_pct, self.clip_upper_pct]
            )
            clipped = valid_z[(valid_z >= low) & (valid_z <= high)]
            if clipped.size < 10:          # 클립 후 픽셀 수 체크
                continue
            Z_raw = float(np.median(clipped))
