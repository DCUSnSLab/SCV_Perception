...
`2.635 m` 기준면 투영으로 매끄럽게 유지하고, 깊이는 별도 거리 영상에
사용합니다. 두 영상 사이의 절단선은 캘리브레이션으로 계산한 `x=1556`의
고정된 세로선입니다. 컬러·깊이 비용으로 절단선을 움직이는 방식은 정지한
물체에서도 경계가 이동해 겉보기 폭을 바꾸므로 기본 경로에서 끕니다.
