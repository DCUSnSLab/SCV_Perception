...
void Detector::beam_init()    // beam initialization
{
    ...
    {
        fi = i * 2 * M_PI / rep;    // 각 beam의 각도 계산

        if (fabs(tan(fi)) > 1)       // "slope" 판단: x와 y의 관계
        {
            beams[i].yx = true;                       // y 방향에 더 정렬됨
            beams[i].d = tan(0.5 * M_PI - fi);          // 1/tan(fi)
            beams[i].o = fabs(off / sin(fi));           // x 방향으로의 half beam-width 투영
        }
        else
        {
            beams[i].yx = false;                      // x 방향에 더 정렬됨
            beams[i].d = tan(fi);                       // tan(fi)
            beams[i].o = fabs(off / cos(fi));           // y 방향으로의 half beam-width 투영
        }
        beamp[i] = &beams[i];   // 미리 할당된 beamp 벡터의 i번째 슬롯에 beams[i]의 포인터 할당
    }

    /*
    // 아래 코드는 인접 beam 간 포인터 연결 (현재 사용하지 않음)
    for (int i = 0, j = 1; j < rep; i++, j++)
    {
        beams[i].l = &beams[j];
        beams[j].r = &beams[i];
    }
    ...
    */
    ...
    // Kfi 값을 rep / (2π)로 설정하여, starShapedSearch에서 fi 값 곱 후 올바른 인덱스를 생성하도록 함
    Kfi = rep / (2 * M_PI);
    ROS_INFO("beam_init(): Initialized %d beams, Kfi set to %f", rep, Kfi);
}
