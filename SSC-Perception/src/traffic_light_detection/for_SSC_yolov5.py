...
import cv2
import numpy as np
import torch  # YOLOv5 모델 로드에 필요한 PyTorch 라이브러fl

# YOLOv5 모델 로드
model = torch.hub.load('ultralytics/yolov5', 'custom', path='/home/marsberry/SSC/src/SSC-Perception/src/traffic_light_detection/traffic_light.pt')
...
frame_threshold = 5  # 미탐지를 방지할 프레임 수 임계값
...
def image_callback(msg):
    ...
    try:
        ...
        # ROI 설정 및 확대
        roi_x, roi_y, roi_w, roi_h = 140, 140, 360, 200  # Adjust ROI coordinates as needed
        scale = 3  # 확대 배율
        ...
        # YOLOv5는 `pandas.DataFrame` 형태로 반환됩니다. 필요한 정보 추출
        detections = results.pandas().xyxy[0]  # 결과는 첫 번째 이미지에 대해 제공됨
