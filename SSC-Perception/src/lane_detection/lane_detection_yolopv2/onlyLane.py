...
def detect():
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        ll_seg_mask_colored[ll_seg_mask == 1] = [255, 255, 255]  # for lane lines
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img:  # Add bbox to image
                        ...
                        # Highlight detected areas in red on the black background
                        x1, y1, x2, y2 = map(int, xyxy)
                        # cv2.rectangle(black_background, (x1, y1), (x2, y2), (0, 0, 255), -1)

                        # # Copy the original detected area to the black background
                        # black_background[y1:y2, x1:x2] = im0[y1:y2, x1:x2]

            # show_seg_result(im0, (np.zeros_like(ll_seg_mask), ll_seg_mask), is_demo=True)
            cv2.imshow('1', black_background)
            cv2.waitKey(1)
            # Save results (image with detections)
            if save_img:
                if dataset.mode == 'image':
                    cv2.imwrite(save_path, black_background)
                    ...
                    vid_writer.write(black_background)
