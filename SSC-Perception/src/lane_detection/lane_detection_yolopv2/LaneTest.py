...
def sliding_window_lane_tracking(binary_warped, nwindows=300, margin=50, minpix=1):
...
class LoadImagesFromBag:
    ...
    def __next__(self):
        ...
        try:
            ...
            self.image = None
            return None, img_resized, img_undistort, None
        except CvBridgeError as e:
            print(f"Error converting ROS Image message to OpenCV image: {e}")
...
def detect():
    ...
    # 버드아이뷰
    src_points = np.float32([[275, 720], [895, 720], [780, 570], [400, 570]])
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # 이미지당 디텍션
            ...
            bev_image = transform_to_bev(black_lane, src_points, dst_points)

            # 슬라이딩 윈도우를 사용한 차선 추적
            leftx, lefty, rightx, righty = sliding_window_lane_tracking(ll_seg_mask)

            # 차선을 그리기
            for x, y in zip(leftx, lefty):
                cv2.circle(testimg, (x, y), 5, (255, 0, 0), -1)
            for x, y in zip(rightx, righty):
                cv2.circle(testimg, (x, y), 5, (0, 0, 255), -1)
            cv2.imshow('1',testimg)
            cv2.imshow('2', im0)
            cv2.waitKey(1)
