...
def detect():
    ...
    for img, im0s, depth_img, camera_info in dataset: # resize, 원본, depth 원본, info
        ...
        ll_only_mask_colored[ll_only_mask == 1] = [255, 0, 0]
        lane_det_img = cv2.addWeighted(im0s, 1, ll_only_mask_colored, 1, 0)
        publisher.publish(masked_depth, camera_info, lane_det_img)

        ll_only_mask_display = (ll_only_mask * 255).astype(np.uint8)  # 흰색/검정색 마스크
        cv2.imshow('Lane Line Segmentation Mask', ll_only_mask_display)
