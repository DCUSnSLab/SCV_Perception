...
class LoadImagesFromBag:
    ...
    def apply_low_pass_filter(self, mask):
        ...
        # 버퍼에 있는 모든 마스크들의 평균을 계산
        filtered_mask = np.mean(self.mask_buffer, axis=0)
        # 필터링된 마스크가 0.5 이상인 경우만 남김
...
def detect():
    ...
    for img, im0s, depth_img, camera_info in dataset: # resize, 원본, depth 원본, info
        ...
        x1_left, y1_left = int(width * 0.4), int(height * 0.6)  # 좌측 아래 파란색 점 근처
        x2_left, y2_left = int(width * 0.6), int(height)  # 좌측 상단 파란색 점 근처
        # x1_right, y1_right = int(width * 0.8), int(height * 0.6)  # 우측 아래 파란색 점 근처
        # x2_right, y2_right = int(width * 0.95), int(height * 0.8)

        middle_mask = np.zeros_like(da_seg_mask)

        middle_mask[y1_left:y2_left, x1_left:x2_left] = 1
        # middle_mask[y1_right:y2_right, x1_right:x2_right] = 0

        da_seg_mask_middle = np.where(middle_mask == 1, da_seg_mask, 0)

        ll_only_mask = np.where(da_seg_mask_middle == 1, 0, ll_seg_mask)

        da_seg_mask_middle_colored = np.zeros_like(im0s)
        da_seg_mask_middle_colored[da_seg_mask_middle == 1] = [255, 255, 255]
        dsmmc_img = cv2.addWeighted(im0s, 1, da_seg_mask_middle_colored, 0.5, 0)
        cv2.imshow('LL', dsmmc_img)
