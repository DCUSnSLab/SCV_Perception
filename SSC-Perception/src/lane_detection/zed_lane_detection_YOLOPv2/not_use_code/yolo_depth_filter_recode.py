...
def detect():
    ...
    out1 = cv2.VideoWriter('testimg.mp4', fourcc, 10, (1280,720))
    out2 = cv2.VideoWriter('testimg2.mp4', fourcc, 10, (1280, 720))
    out3 = cv2.VideoWriter('im0s.mp4', fourcc, 10, (1280, 720))

    for img, im0s, depth_img, camera_info in dataset: # resize, original, depth original, info
        print(im0s.shape[1], im0s.shape[0])
        ...
        black_ll = cv2.addWeighted(black, 1, ll_only_mask_colored, 1, 0)
        cv2.imshow('LL Only Mask on Depth', black_ll)
        ...
        out1.write(black_ll)
        out2.write(testimg_ll_only)
        out3.write(black_mask)
