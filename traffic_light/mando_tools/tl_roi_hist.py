...
class TLCropOnlyNode(Node):
    def __init__(self):
        ...
        # ---------------- Params ----------------
        # I/O
        self.declare_parameter('model_path', default_tl_model_path())
        self.declare_parameter('image_topic', default_runtime_image_topic())
        self.declare_parameter('pub_hist_image', False)
        self.declare_parameter('show_windows', False)
        self.declare_parameter('max_fps', 15.0)

        # Preferred window for selecting the main signal candidate
        ...
        # YOLO detection window and runtime
        self.declare_parameter('detect_top_ratio',    0.00)
        self.declare_parameter('detect_bottom_ratio', 1.00)
        self.declare_parameter('detect_left_ratio',   0.00)
        self.declare_parameter('detect_right_ratio',  1.00)
        self.declare_parameter('detector_conf_threshold', 0.10)
        self.declare_parameter('detector_iou_threshold', 0.45)
        self.declare_parameter('detector_image_size', 640)
        self.declare_parameter('detector_max_detections', 50)
        self.declare_parameter('detector_device', 'cuda:0')
        ...
        self.detect_top    = float(self.get_parameter('detect_top_ratio').value)
        self.detect_bottom = float(self.get_parameter('detect_bottom_ratio').value)
        self.detect_left   = float(self.get_parameter('detect_left_ratio').value)
        self.detect_right  = float(self.get_parameter('detect_right_ratio').value)
        self.detector_conf_threshold = float(
            self.get_parameter('detector_conf_threshold').value
        )
        self.detector_iou_threshold = float(
            self.get_parameter('detector_iou_threshold').value
        )
        self.detector_image_size = int(self.get_parameter('detector_image_size').value)
        self.detector_max_detections = int(
            self.get_parameter('detector_max_detections').value
        )
        self.detector_device_request = str(self.get_parameter('detector_device').value)
        self.detector_device = resolve_inference_device(self.detector_device_request)
        ...
        self.get_logger().info(
            'Inference device: '
            f'{self.detector_device} (requested: {self.detector_device_request})'
        )
    ...
    def _window_from_ratios(self, shape, left, right, top, bottom):
        H, W = shape[:2]
        x0 = int(W * left)
        x1 = int(W * right)
        y0 = int(H * top)
    ...
    def _preferred_window(self, shape):
        return self._window_from_ratios(
            shape,
            self.roi_left,
            self.roi_right,
            self.roi_top,
    ...
    def _detection_window(self, shape):
        return self._window_from_ratios(
            shape,
            self.detect_left,
            self.detect_right,
    ...
    def _box_center_in_window(self, box, window):
        cx, cy = self._box_center(box)
        x0, y0, x1, y1 = window
        return x0 <= cx <= x1 and y0 <= cy <= y1

    def _box_tracking_score(self, box, reference_box, shape):
        if reference_box is None:
            return 1.0
        cx0, cy0 = self._box_center(box)
        cx1, cy1 = self._box_center(reference_box)
        H, W = shape[:2]
        diag = max(1.0, math.hypot(W, H))
        dist = math.hypot(cx0 - cx1, cy0 - cy1)
        return float(clamp(1.0 - (dist / diag), 0.0, 1.0))
    ...
    def _yolo_boxes_filtered(self, det_shape, results, x_offset=0, y_offset=0):
        H, W = det_shape[:2]
        boxes = []
    ...
    def _select_box(self, shape, boxes):
        if not boxes:
            return None

        H, W = shape[:2]
        preferred_window = self._preferred_window(shape)
        preferred = []
        scored = []

        for item in boxes:
            conf = item['conf']
            xA, yA, xB, yB = item['box']
            w = max(1, xB - xA)
            h = max(1, yB - yA)

            cy = 0.5 * (yA + yB)
            cy_n = float(cy) / float(max(1, H))
            vy = 1.0 - cy_n
            area_n = (w * h) / float(max(1, W * H))

            score = float(conf) * (0.5 + 0.5 * vy) * (0.5 + 0.5 * area_n)
            score *= (0.8 + 0.2 * self._box_tracking_score(
                item['box'],
                self.last_selected_box,
                shape,
            ))

            scored_item = (score, item)
            scored.append(scored_item)
            if self._box_center_in_window(item['box'], preferred_window):
                preferred.append(scored_item)

        candidates = preferred if preferred else scored
        candidates.sort(key=lambda t: t[0], reverse=True)
        return candidates[0][1]

    def _detect_boxes(self, frame):
        x0, y0, x1, y1 = self._detection_window(frame.shape)
        if x1 <= x0 or y1 <= y0:
            return [], (x0, y0, x1, y1)

        det_frame = frame[y0:y1, x0:x1]
        ...
        boxes = self._yolo_boxes_filtered(det_frame.shape, results, x_offset=x0, y_offset=y0)
        return boxes, (x0, y0, x1, y1)

    def _resolve_detector_classes(self):
        names = None
        try:
            if hasattr(self.model, 'model') and hasattr(self.model.model, 'names'):
        ...
        if isinstance(names, dict):
            ...
            normalized = {i: str(v).strip().lower() for i, v in enumerate(names)}
        else:
            normalized = {}

        traffic_light_classes = [
            idx for idx, name in normalized.items()
            if name == 'traffic light' or name.startswith('vehicular_')
        ]
        return traffic_light_classes or None
    ...
    def _process_latest_frame(self):
        ...
        try:
            ...
            pref_x0, pref_y0, pref_x1, pref_y1 = self._preferred_window(frame.shape)
            det_x0, det_y0, det_x1, det_y1 = self._detection_window(frame.shape)
            if pref_x1 > pref_x0 and pref_y1 > pref_y0:
                cv2.rectangle(frame_dbg, (pref_x0, pref_y0), (pref_x1, pref_y1), (0, 255, 255), 1)
            if det_x1 > det_x0 and det_y1 > det_y0:
                cv2.rectangle(frame_dbg, (det_x0, det_y0), (det_x1, det_y1), (255, 128, 0), 1)

            for item in boxes:
                xA, yA, xB, yB = item['box']
                cv2.rectangle(frame_dbg, (xA, yA), (xB, yB), (0, 128, 255), 1)

            selected = self._select_box(frame.shape, boxes)
            has_box = selected is not None
            if has_box:
                self.last_seen_box_ns = t_ns
                xA, yA, xB, yB = selected['box']
                self.last_selected_box = selected['box']
                cv2.rectangle(frame_dbg, (xA, yA), (xB, yB), (0, 255, 0), 2)
                label = f"{selected['class_name']} {selected['conf']:.2f}"
                cv2.putText(
                    frame_dbg,
                    label,
                    (xA, max(16, yA - 6)),
                ...
                xA = clamp(xA + self.margin, 0, frame.shape[1]-1)
                yA = clamp(yA + self.margin, 0, frame.shape[0]-1)
                xB = clamp(xB - self.margin, 0, frame.shape[1]-1)
                yB = clamp(yB - self.margin, 0, frame.shape[0]-1)
                if xB <= xA or yB <= yA:
                    has_box = False

            if has_box:
                zoom = frame[yA:yB, xA:xB].copy()
            ...
            # ---------- 상태 판정 ----------
            proposed = 0  # 0=UNK, 1=R, 2=Y, 3=G, 4=LEFT ARROW
            ...
            try:
                self.pub_roi_dbg.publish(self.bridge.cv2_to_imgmsg(frame_dbg, 'bgr8'))
            except Exception as e:
