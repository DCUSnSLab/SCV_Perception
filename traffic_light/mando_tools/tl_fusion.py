...
class TLFusionNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.fallback_red_h_max = float(self._declare_param('fallback_red_h_max', 8.0))
        self.fallback_red_h_wrap_min = float(
            self._declare_param('fallback_red_h_wrap_min', 170.0)
        )
        self.fallback_red_v_min = int(self._declare_param('fallback_red_v_min', 85))
        self.fallback_green_h_min = float(self._declare_param('fallback_green_h_min', 39.0))
        self.fallback_green_h_max = float(self._declare_param('fallback_green_h_max', 90.0))
        ...
        self.fallback_green_middle_v_min = int(
            self._declare_param('fallback_green_middle_v_min', 60)
        )
        ...
        self.fallback_green_middle_weight = float(
            self._declare_param('fallback_green_middle_weight', 1.50)
        )
        self.fallback_middle_mask_dilate_iterations = int(
            self._declare_param('fallback_middle_mask_dilate_iterations', 1)
        )
        ...
        *,
        ...
        if not self.use_torch_color_fallback:
            ...
            red_mask_1 = cv2.inRange(
                hsv,
                (0, self.fallback_s_min, self.fallback_red_v_min),
                (int(self.fallback_red_h_max), 255, 255),
            )
            red_mask_2 = cv2.inRange(
                hsv,
                (int(self.fallback_red_h_wrap_min), self.fallback_s_min, self.fallback_red_v_min),
                (179, 255, 255),
            )
            ...
            middle_region = self._middle_mask_region(signal_mask, enhanced.shape[:2])
            if middle_region is not None:
                green_middle_mask = cv2.inRange(
                    hsv,
                    (
                        self.fallback_green_h_min,
                        self.fallback_green_s_min,
                        self.fallback_green_middle_v_min,
                    ),
                    (self.fallback_green_h_max, 255, 255),
                )
                green_mask = cv2.bitwise_or(
                    green_mask,
                    cv2.bitwise_and(green_middle_mask, middle_region),
                )
            ...
            red_mask = self._amplify_middle_mask(red_mask, signal_mask)
            yellow_mask = self._amplify_middle_mask(yellow_mask, signal_mask)
            green_mask = self._amplify_middle_mask(green_mask, signal_mask)
    ...
    def _torch_color_measurements(
        ...
        with torch.inference_mode():
            ...
            if color_reference is None:
                ...
                enhanced_bgr = torch.from_numpy(np.ascontiguousarray(enhanced)).to(
                    device=self.color_fallback_device,
                    non_blocking=True,
                )
            ...
            red_mask = (
                ((hue >= 0.0) & (hue <= self.fallback_red_h_max))
                | ((hue >= self.fallback_red_h_wrap_min) & (hue < 180.0))
            )
            red_mask = (
                red_mask
                & (saturation >= self.fallback_s_min)
                & (value >= self.fallback_red_v_min)
            ...
            green_middle_mask = (
                (hue >= self.fallback_green_h_min)
                & (hue <= self.fallback_green_h_max)
                & (saturation >= self.fallback_green_s_min)
                & (value >= self.fallback_green_middle_v_min)
            )
            masks_gpu = {
            ...
            if signal_mask is not None:
                ...
                middle_region = self._middle_mask_region(signal_mask, enhanced.shape[:2])
                if middle_region is not None:
                    middle_region_tensor = torch.from_numpy(middle_region).to(
                        device=bgr.device,
                        non_blocking=True,
                    ) > 0
                    masks_gpu['green'] = masks_gpu['green'] | (
                        green_middle_mask & middle_region_tensor & signal_region
                    )
                masks_gpu = {
                    name: self._torch_amplify_middle_mask(
                        mask, signal_mask, enhanced.shape[:2],
                    )
    ...
    def _middle_mask_region(
        ...
        if weight_rows is None:
            return None
        top_row, bottom_row = weight_rows
        row_indices = np.arange(shape[0], dtype=np.float32)
        if bottom_row == top_row:
        ...
        region[middle_rows, :] = signal_mask[middle_rows, :]
        return region

    def _amplify_middle_mask(
        self,
        mask: np.ndarray,
        signal_mask: np.ndarray | None,
    ) -> np.ndarray:
        if self.fallback_middle_mask_dilate_iterations <= 0:
            return mask
        region = self._middle_mask_region(signal_mask, mask.shape[:2])
        if region is None or np.count_nonzero(mask) == 0:
            return mask
        dilated = cv2.dilate(
            mask,
            self.fallback_kernel,
            iterations=self.fallback_middle_mask_dilate_iterations,
        )
        return cv2.bitwise_or(mask, cv2.bitwise_and(dilated, region))

    def _torch_amplify_middle_mask(
        self,
        mask: Any,
        signal_mask: np.ndarray | None,
        shape: tuple[int, int],
        ...
        if self.fallback_middle_mask_dilate_iterations <= 0:
            return mask
        region = self._middle_mask_region(signal_mask, shape)
        if region is None:
            return mask
        dilated = mask
        for _ in range(self.fallback_middle_mask_dilate_iterations):
            dilated = self._torch_ellipse_reduce(
                dilated, reduce='max', border_value=0.0,
            )
        middle_region = torch.from_numpy(region).to(
            device=mask.device,
            non_blocking=True,
        ) > 0
        return mask | (dilated & middle_region)
