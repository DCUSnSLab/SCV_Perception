...
OPENPCDET_PATH = Path(
    os.environ.get(
        "OPENPCDET_PATH",
        Path(__file__).resolve().parents[2] / "third_party" / "OpenPCDet",
    )
).expanduser().resolve()
...
def _default_ckpt_file() -> Path:
    """Resolve the PointPillars checkpoint path.

    The model lives in the source tree (jay_tracker/models) since pcdet itself
    is imported from there; the install tree carries no models/ dir. Prefer an
    explicit env override, then the installed convention, then the source tree.
    """
    env = os.environ.get("CODA_CKPT_FILE")
    if env:
        return Path(env).expanduser().resolve()
    candidates = [
        # Installed alongside the package: lib/python3.10/models
        Path(__file__).resolve().parents[2] / "models" / "pointpillars_coda.pth",
        # Source checkout: jay_tracker/models, sibling of third_party/OpenPCDet
        Path(pcdet.__file__).resolve().parents[3] / "models" / "pointpillars_coda.pth",
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate.resolve()
    # Return the source-tree location so the error message points somewhere sane.
    return candidates[-1]


CLASS_COLORS = {
    "Vehicle": (0.10, 0.80, 0.25),
    "Pedestrian": (1.00, 0.55, 0.10),
}
...
class CodaPointPillarNode(Node):
    def __init__(self):
        super().__init__("coda_pointpillar_node")
        ...
        self.declare_parameter("log_distances", True)
        ...
        self.declare_parameter(
            "cfg_file",
            str(
                Path(get_package_share_directory("pointpillars_coda_detector")) / "config" / "coda_pointpillar_vehicle_ped.yaml"
            ),
        )
        self.declare_parameter(
            "ckpt_file",
            str(_default_ckpt_file()),
        )
        ...
        self.log_distances = bool(self.get_parameter("log_distances").value)
        self.frame_count = 0

        self.cfg_file = str(Path(self.get_parameter("cfg_file").value).expanduser().resolve())
        self.ckpt_file = str(Path(self.get_parameter("ckpt_file").value).expanduser().resolve())
        ...
        self.get_logger().info(f"OPENPCDET_PATH={OPENPCDET_PATH}")
        self.get_logger().info(f"Loading config: {self.cfg_file}")
        ...
        self.get_logger().info(f"Loading checkpoint: {self.ckpt_file}")
        ...
        self.get_logger().info(
            ...
            f"markers={self.marker_topic}, score_thresh={self.score_thresh:.2f}, z_offset={self.z_offset:.2f}"
        )
    ...
    def read_points(self, msg):
        ...
        points = self._read_points_fast(msg, intensity_field)
        if points is None:
    ...
    def _read_points_fast(self, msg, intensity_field):
        if msg.is_bigendian:
            return None
        ...
        required = ["x", "y", "z"]
        if intensity_field:
            required.append(intensity_field)
        ...
        offsets = {name: field_map[name].offset for name in required}
        raw = np.frombuffer(msg.data, dtype=np.uint8)
        if raw.size != msg.row_step * msg.height:
            return None

        rows = raw.reshape(msg.height, msg.row_step)
        points = np.empty((msg.width * msg.height, 4), dtype=np.float32)
        cursor = 0

        for row_idx in range(msg.height):
            row = rows[row_idx, : msg.width * msg.point_step]
            row_points = row.reshape(msg.width, msg.point_step)
            xyz = np.stack(
                [
                    row_points[:, offsets["x"] : offsets["x"] + 4].view(np.float32).reshape(-1),
                    row_points[:, offsets["y"] : offsets["y"] + 4].view(np.float32).reshape(-1),
                    row_points[:, offsets["z"] : offsets["z"] + 4].view(np.float32).reshape(-1),
                ],
                axis=1,
            )
            count = xyz.shape[0]
            points[cursor : cursor + count, :3] = xyz
            if intensity_field:
                points[cursor : cursor + count, 3] = (
                    row_points[:, offsets[intensity_field] : offsets[intensity_field] + 4]
                    .view(np.float32)
                    .reshape(-1)
                )
            else:
                points[cursor : cursor + count, 3] = 0.0
            cursor += count
    ...
    def lidar_callback(self, msg):
        ...
        if self.log_distances and len(pred_boxes) > 0:
            parts = []
            for i in range(len(pred_boxes)):
                label_idx = int(pred_labels[i]) - 1
                cls_name = self.class_names[label_idx] if 0 <= label_idx < len(self.class_names) else "Unknown"
                bx, by, bz = float(pred_boxes[i][0]), float(pred_boxes[i][1]), float(pred_boxes[i][2])
                dist = math.sqrt(bx * bx + by * by + bz * bz)
                parts.append(f"{cls_name}@{dist:.2f}m(score={float(pred_scores[i]):.2f})")
            self.get_logger().info(f"sensor->object distances: {', '.join(parts)}")
    ...
    def publish_markers(self, boxes, scores, labels, header):
        ...
        for i, box in enumerate(boxes):
            label_idx = int(labels[i]) - 1
            cls_name = self.class_names[label_idx] if 0 <= label_idx < len(self.class_names) else "Unknown"
            color = CLASS_COLORS.get(cls_name, (0.2, 0.9, 1.0))
            ...
            dist = math.sqrt(float(box[0]) ** 2 + float(box[1]) ** 2 + float(box[2]) ** 2)
